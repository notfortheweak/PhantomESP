...
// View-scoped row cache, heap-backed: wd_seen_t[MAX_ROWS] held statically was
...
static wd_seen_t *s_rowsig = NULL;
...
static void wardrive_list_create(void) {
    ...
    s_nrows = 0;
    if (!s_rowsig) {
        s_rowsig = heap_caps_calloc(MAX_ROWS, sizeof(wd_seen_t),
                                 MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (!s_rowsig) s_rowsig = heap_caps_calloc(MAX_ROWS, sizeof(wd_seen_t),
